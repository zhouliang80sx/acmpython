# -*- coding: utf-8 -*-
"""
Created on Wed Aug 15 08:13:15 2018
...
class HumanPlayer(Player):
 ...
 def think(self, board):
  while True:
   action = int(input('Please input a num in 0-8:'))
   if (action<N*N and action>0) and board.is_legal_action(int(action)):
    return int(action)
   if action=='-1':
       sys.exit()
...
class AIPlayer(Player):
 ...
 def think(self, board):
  ...
  take = ['X','O'][self.take=='X'] #列表索引简化的条件语句，不推荐这么写code
